...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	UECIndicatorManagerComponent* IndicatorComponent = IndicatorComponentPtr.Get();
	if (IndicatorComponent == nullptr)
	{
		IndicatorComponent = UECIndicatorManagerComponent::GetComponent(LocalPlayerContext.GetPlayerController());
		if (IndicatorComponent)
		{
			...
			// 상호작용 어빌리티에 의해 UIndicatorDescriptor가 등록/제거될 때 실행할 함수를 바인딩합니다.
	...
	if (LocalPlayer)
	{
		...
		if (LocalPlayer->GetProjectionData(LocalPlayer->ViewportClient->Viewport, /*out*/ ProjectionData))
		{
			...
			for (int32 ChildIndex = 0; ChildIndex < CanvasChildren.Num(); ++ChildIndex)
			{
				...
				// Descriptor가 지정한 투영 방식에 따라 대상의 월드 위치를 화면 XY 좌표와 카메라 거리 Z로 변환합니다.
...
void SActorCanvas::AddIndicatorForEntry(UIndicatorDescriptor* Indicator)
{
	...
	// AsyncMixin 없이 동기로 로드합니다.
	UClass* LoadedClass = IndicatorClass.LoadSynchronous();
	if (!LoadedClass)
	{
		UE_LOG(LogTemp, Warning, TEXT("Failed to load indicator widget class."));
		return;
	}

	TSubclassOf<UUserWidget> LoadedWidgetClass = LoadedClass;

	UUserWidget* IndicatorWidget = IndicatorPool.GetOrCreateInstance(LoadedWidgetClass);

	if (!IndicatorWidget)
	{
		UE_LOG(LogTemp, Warning, TEXT("Failed to create indicator widget."));
		return;
	}
