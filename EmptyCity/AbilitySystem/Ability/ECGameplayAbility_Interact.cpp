...
void UECGameplayAbility_Interact::UpdateInteractions(const TArray<FInteractionOption>& InteractiveOptions)
{
	...
	{	
		// InteractiveOptions를 받아서 UI를 화면에 표시
        
		if (UECIndicatorManagerComponent* IndicatorManager = UECIndicatorManagerComponent::GetComponent(PC))
		{
			...
			for (const FInteractionOption& InteractionOption : InteractiveOptions)
			{
				...
				Indicator->SetMainText(InteractionOption.Text);
				Indicator->SetSubText(InteractionOption.SubText);
